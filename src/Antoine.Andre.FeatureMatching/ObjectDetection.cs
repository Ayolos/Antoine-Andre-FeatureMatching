...
namespace Antoine.Andre.FeatureMatching
{
    public class ObjectDetection
    {
        public Task<IList<ObjectDetectionResult>> DetectObjectInScenesAsync(byte[] objectImageData, IList<byte[]> imagesSceneData)
        {
            var tasks = imagesSceneData.Select(imageSceneData => Task.Run(() => DetectObjectInScene(objectImageData, imageSceneData))).ToList();

            return Task.WhenAll(tasks).ContinueWith(t => (IList<ObjectDetectionResult>)t.Result.ToList());
        }
        
        public async Task<IList<ObjectDetectionResult>> DetectObjectInScenesAsyncMock(byte[] objectImageData, IList<byte[]> imagesSceneData)
        {
            IList<ObjectDetectionResult> results = new List<ObjectDetectionResult>(); results.Add(new ObjectDetectionResult
            { ImageData = new byte[]{0},
                Points = new List<ObjectDetectionPoint> { new() {X=1,Y=2} }} );
            results.Add(new ObjectDetectionResult { ImageData = new byte[]{0},
                Points = new List<ObjectDetectionPoint> { new() {X=1,Y=2} }} ); return results;
        }
        
        private ObjectDetectionResult DetectObjectInScene(byte[] imageObjectData, byte[] imageSceneData)
        {
            using var imgobject = Mat.FromImageData(imageObjectData, ImreadModes.Color); using var imgScene = Mat.FromImageData(imageSceneData, ImreadModes.Color);
            using var orb = ORB.Create(10000);
            using var descriptors1 = new Mat();
            using var descriptors2 = new Mat();
            orb.DetectAndCompute(imgobject, null, out var keyPoints1, descriptors1); orb.DetectAndCompute(imgScene, null, out var keyPoints2, descriptors2);
